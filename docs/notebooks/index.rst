...
These notebooks demonstrate BADC workflows using the public ``bogus`` dataset. Keep them
lightweight (≤10 MB inputs, stub inference by default) so contributors can run them on laptops or CI
workers without GPUs.

Layout overview:

``notebooks/chunk_probe.ipynb``
   Explore ``badc chunk probe`` / ``badc chunk split`` and visualize segment plans.
``notebooks/infer_local.ipynb``
   Run ``badc infer run`` in stub mode, inspect dataset-aware outputs, and aggregate results.
``notebooks/aggregate_analysis.ipynb``
...
* Mark GPU-heavy cells with ``if USE_HAWKEARS:`` guards or convert them into ``badc --print-datalad-run``
